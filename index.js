...
['Feb-2017', 671099]
  ...
  var earningsArray = data.map((el) => el[1]);
  
  var profitMonths = data.filter((el) => el[1] > 0);
  var salesOnProfitMonths = profitMonths
    .map((el) => el[1])
    .reduce((accVal, curVal) => accVal + curVal, 0);
  
  var avgOfProfitAndLoss =
    earningsArray.reduce((accVal, curVal) => accVal + curVal, 0) / data.length; 
...
// console.log(getOccurrence(finances, 1));  // 2
// console.log(getOccurrence(finances, 3));  // 3